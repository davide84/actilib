...
def get_dprime_default_params():
    return {
        ...
        "diameter_mm": 10,           # lesion diameter for simulations [mm]
...
def calculate_radial_mesh(params):
    fov_mm = params["px_number"] * params["px_size_mm"]
    x = [(i * params['px_size_mm'] - fov_mm / 2.0) for i in range(params['px_number'])]
...
def get_eye_filter(freq_1d, params):
    if params['view_filter'] == 'NPWE':
        ...
        fov_mm = params["px_number"] * params["px_size_mm"]
        display_mm = fov_mm * params['view_zoom']
...
def calculate_dprime(data_freq, data_nps, data_ttf, params=get_dprime_default_params()):
    ...
    internal_noise = np.zeros(params['px_number'])  # TODO implement noise calculation instead of null matrix
    freq_spacing_coeff = (1.0 / (params['px_size_mm'] * params['px_number'])) ** 2
